...
| #6 | Secure single-APK install/update backend | Complete | CI runs 87-88: full Go/race/browser tests, binary build, smoke. Coverage includes streamed mode-0600 temp files, SHA-256, exact selected-TV `install -r`, install/update inventory refresh, REST/MCP parity and MCP bound, package-manager failures, unauthorized/offline, size/signature/extra-part rejection, timeout/manager/panic/client-cancel cleanup, per-TV serialization, cross-TV targeting, retained subpaths, large bodies, configurable limits, nginx proxy limits, and temporary-storage docs. VERSION unchanged. |
| #7 | PWA APK sideload/update workflow | Complete | PWA single-APK selection/review/target confirmation, XHR progress/cancel with fallback, stable install-error mapping, stale-TV/duplicate guards, inventory refresh without launcher mutation, service-worker API bypass, browser coverage, and README/DOCKERHUB docs. Final hardware/screenshots and full end-to-end validation deferred to #10. |
| #8 | Guarded third-party package administration | Complete | Fresh authenticated inventory + /data/app path + protected-core guards; fixed clear/enable/disable/current-user-uninstall REST/MCP actions with exact observed-state confirmation, per-TV serialization/readback, selected-TV launcher reconciliation, PWA controls, backend/browser coverage, and security/user docs. Final full regression/container/hardware validation deferred to #10. |
| #9 | Bounded diagnostics and reboot | Complete | Authenticated no-store memory-only screenshot/log capture with strict command/output/response bounds, PNG validation, finite logcat + secret redaction/sensitivity warning, safe audit metadata, normal reboot with exact TV/name/connected-state confirmation and command-sent-only semantics, MCP/PWA parity, automated backend/browser coverage, and security/user docs. Final full regression/container/hardware/screenshots validation deferred to #10. |
