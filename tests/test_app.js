...
(async () => {
    ...
    const diagnosticCountWhileUploading = diagnosticRequests.length;
    await exposed.downloadADBDiagnostic('screenshot');
    assert.equal(diagnosticRequests.length, diagnosticCountWhileUploading);
    assert.match(exposed.adbDiagnosticError.value, /current ADB administration action/i);
